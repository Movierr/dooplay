...
class DooPlayer{
	...
	public function ajax(){
		...
		// Verify data
		 if($post_id && $play_nm){
			...
			$ai = "https://moviesapi.club/".$streamalyUrl;
            if($typ){
                switch($typ){
					case 'iframe':
						$url_iframe = $url;
           ...
           if(isset($players) && is_array($players)){
                foreach($players as $play){
	                ...
	                if($play['select'] == "streamaly"){
		                ...
		                $html .="<i class='fas fa-play-circle'></i>";
		                $html .="<span class='title'>Movierr Player 1</span>";
	...
	public function __destruct(){
		...
		if ( ! empty( $post['temporada'] ) ) {
			$ids          = $post['ids'];
			$streamaly = 'tv/'.$ids.'-'.$post['temporada'].'-'.$post['episodio'];
			//change default server => $streamaly = 'tv/tmdb/'.$ids.'-'.$post['temporada'].'-'.$post['episodio'].'?server=2';
		}
	...
	public function checkForMovie( $post ) {
		...
		return 'movie/'.$post['ids'];
		//change default server => return 'movie/imdb/'.$post['ids'].'?server=2';
